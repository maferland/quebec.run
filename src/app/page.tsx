    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-xl text-gray-600 max-w-2xl mx-auto">
          Find your running community and join exciting runs throughout the city. 
          Connect with fellow runners and explore new routes together.
        </p>
      </div>
            ...
            <div key={club.id} className="bg-white p-6 rounded-lg shadow-md">
              ...
              {club.runs.length > 0 && (
                <div className="border-t pt-3">
                  <h4 className="font-medium text-gray-900 mb-2">Upcoming runs:</h4>
                  {club.runs.map((run) => (
                    <div key={run.id} className="text-sm text-gray-600 mb-1">
                      <strong>{run.title}</strong> - {format(new Date(run.date), 'MMM dd, yyyy')} at {run.time}
                    </div>
                  ))}
                </div>
              )}
      ...
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <p className="text-gray-600 mb-4">
            Explore run clubs and upcoming events on an interactive map of Quebec City.
          </p>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-2xl font-semibold text-gray-900 mb-4">Calendar View</h2>
          <p className="text-gray-600 mb-4">
